...
function gen_header() {
	global $Version;
	$title = "METAR Wind Gusts - Nationwide";
...
function gen_entry($M, $miles, $bearingWR) {
    ...
    */
    ...
    if (isset($M['dwindgust'])) {
      $gustSpeed = $M['dwindgust'];
      $gustColor = "";
  
      if ($gustSpeed <= 20) {
        $gustColor = "0 255 0"; // green 
      } elseif ($gustSpeed <= 40) {
        $gustColor = "255 255 0"; // yellow 
      } elseif ($gustSpeed <= 60) {
        $gustColor = "255 165 0"; // orange 
      } elseif ($gustSpeed <= 70) {
        $gustColor = "255 0 0"; // red 
      } else {
        $gustColor = "255 0 255"; // purple for very strong
      }
  
      $popup = gen_popup($M);
  
      $output .= "Color: $gustColor\n";
      $output .= "Text: 0, 0, 1, $gustSpeed, \"" . gen_popup($M) . "\"\n";;
    }

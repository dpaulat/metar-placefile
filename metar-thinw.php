...
function gen_header() {
	global $Version;
	$title = "METAR Temperatures/Heat Index - Nationwide";
...
function gen_entry($M, $miles, $bearingWR) {
  ...
  */
  ...
  if (isset($M['dtemp']) && isset($M['dheatidx']) && $M['dheatidx'] >= 103) {
      $tempF = $M['dtemp'];
      $heatIdx = $M['dheatidx'];

      $tempColor = "";
      if ($tempF <= 32) {
          $tempColor = "0 0 255";  // blue for cold
      } elseif ($tempF <= 50) {
          $tempColor = "0 128 255";  // lighter blue for cool
      } elseif ($tempF <= 69) {
          $tempColor = "0 255 0";  // green for mild
      } elseif ($tempF <= 85) {
          $tempColor = "255 165 0";  // orange for warm
      } elseif ($tempF < 100) {
          $tempColor = "255 0 0";  // red for hot
      } else {
          $tempColor = "255 105 180";  // pink for 100 and above
      }

...
function gen_header() {
	global $Version;
	$title = "METAR Temperatures - Nationwide";
...
*/	
  ...
  if(isset($M['dtemp'])) {
    ...
    } elseif ($tempF > 32 && $tempF <= 50) {
        $output .= "Color: 0 128 255\n";  // lighter blue for cool
        $output .= "Text: 0, 0, 1, ".$tempF.",\"".gen_popup($M)."\"\n";
    } elseif ($tempF > 50 && $tempF <= 69) {
        $output .= "Color: 0 255 0\n";  // green for mild
        $output .= "Text: 0, 0, 1, ".$tempF.",\"".gen_popup($M)."\"\n";
    } elseif ($tempF > 69 && $tempF <= 85) {
        $output .= "Color: 255 165 0\n";  // orange for warm
        $output .= "Text: 0, 0, 1, ".$tempF.",\"".gen_popup($M)."\"\n";
    } elseif ($tempF > 85 && $tempF < 100) {
        $output .= "Color: 255 0 0\n";  // red for hot
        $output .= "Text: 0, 0, 1, ".$tempF.",\"".gen_popup($M)."\"\n";
    } else {
        $output .= "Color: 255 105 180\n";  // pink for 100 and above
        $output .= "Text: 0, 0, 1, ".$tempF.",\"".gen_popup($M)."\"\n";
    }
  }
...
//   $dewpt = $M['ddewpt'];
//   if ($dewpt <= 9) {
//       $output .= "Color: 80 80 255\n"; // Blue
//   } elseif ($dewpt >= 10 && $dewpt <= 19) {
//       $output .= "Color: 80 148 255\n"; // Light Blue
//   } elseif ($dewpt >= 20 && $dewpt <= 29) {
//       $output .= "Color: 148 255 255\n"; // Cyan
//   } elseif ($dewpt >= 30 && $dewpt <= 39) {
//       $output .= "Color: 148 255 148\n"; // Light Green
//   } elseif ($dewpt >= 40 && $dewpt <= 49) {
//       $output .= "Color: 255 255 80\n"; // Yellow
//   } elseif ($dewpt >= 50 && $dewpt <= 59) {
//       $output .= "Color: 255 217 80\n"; // Light Orange
//   } elseif ($dewpt >= 60 && $dewpt <= 69) {
//       $output .= "Color: 255 183 80\n"; // Orange
//   } elseif ($dewpt >= 70 && $dewpt <= 79) {
//       $output .= "Color: 255 148 80\n"; // Dark Orange
//   } elseif ($dewpt >= 80 && $dewpt <= 89) {
//       $output .= "Color: 255 114 80\n"; // Red-Orange
//   } elseif ($dewpt >= 90 && $dewpt <= 99) {
//       $output .= "Color: 217 80 80\n"; // Red
//   } elseif ($dewpt >= 100 && $dewpt <= 109) {
//       $output .= "Color: 183 80 80\n"; // Dark Red
//   } else {
//       $output .= "Color: 148 80 80\n"; // Darker Red
//   }
//   $output .= "Text: -17, -13, 1, ".$dewpt.",\"".gen_popup($M)."\"\n";
// }
// 	if(isset($M['dvis'])) {
...
function gen_popup($M) {
	...
	$out .= "-----------------------------------------------------".'\n';
    // Split RAW-METAR into three lines
    $rawMetar = $M['RAW-METAR'];
    $length = strlen($rawMetar);
    $firstThird = intval($length / 3);
    $secondThird = intval(2 * $length / 3);

    $firstSpace = strpos($rawMetar, ' ', $firstThird);
    $secondSpace = strpos($rawMetar, ' ', $secondThird);

    if ($firstSpace !== false && $secondSpace !== false) {
        $firstLine = substr($rawMetar, 0, $firstSpace);
        $secondLine = substr($rawMetar, $firstSpace + 1, $secondSpace - $firstSpace - 1);
        $thirdLine = substr($rawMetar, $secondSpace + 1);
        $out .= $firstLine . '\n' . $secondLine . '\n' . $thirdLine . '\n';
    } else {
        $out .= $rawMetar . '\n'; // If spaces aren't found, keep it as one line
    }
	$out .= "-----------------------------------------------------".'\n';
  ...
  if(isset($M['HEATINDEX'])) {
		$out .="HIdx: ".$M['HEATINDEX'].'\n';
	}
	$out .= "DewP: ".$M['DEWPT'].'\n';
	$out .= "RelH: ".$M['HUMIDITY'].'\n';
	
  if(strpos($M['WIND'], ', gusting') !== false) {
  ...
  if(strlen($M['CLOUD-DETAILS']) > 4) {
    $sky = str_replace("\t", '\n      ', $M['CLOUD-DETAILS']); // Replace tabs with newlines
    $sky = rtrim($sky, ','); // Remove trailing comma if it exists
    $out .= "Sky:  ".$sky.'\n';
}
	if($M['dalthpa'] > 500) {
